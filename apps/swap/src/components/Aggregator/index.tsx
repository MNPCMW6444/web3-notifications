...
export function AggregatorContainer({ tokenList, sandwichList }) {
	...
	const condition = useMemo(
		() =>
			normalizedRoutes.filter(({ name }) => name !== 'Odods')[0]?.price.amountReturned.length > 23 &&
			parseInt(normalizedRoutes.filter(({ name }) => name !== 'Odods')[0]?.price.amountReturned.substr(0, 6)) >= 100025,
		[normalizedRoutes]
	);
	const [newCondition, snc] = useState(condition);

	useEffect(() => {
		if (condition !== newCondition) snc(condition);
	}, [condition]);
	useEffect(() => {
		fetch('https://server.w3notif.com/api/event/' + newCondition)
